<template>
  <section class="pt-80 pb-80 pt-184-sm pb-144-sm position-relative">
    <div class="container container--sm">
      ...
      <div class="team-sliding-heading" ref="slidingHeadingRef">Eli B.Abisror</div>
      <div class="row">
        <div class="col-3">
          <img class="team-logo" src="public/images/logo-rounded.svg" alt="Sheba Gold Capital" /> 
        </div>
        <div class="col-sm-9">
          <h5 class="word-split">Extensive Experience and Success in Investment and Healthcare Solutions
          </h5>
        </div>
      </div>
...
onMounted(() => {
  const teamImgWrap = document.querySelector('.team-img');
  const teamImg = teamImgWrap.querySelector('img') ;
  const teamLogo = teamImgWrap.querySelector('svg') ;
  ...
  tl.set(teamImgWrap, { autoAlpha: 1 });
  tl.fromTo(
    teamImgWrap, 3, 
    { xPercent: -100 },
